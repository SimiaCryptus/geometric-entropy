...
function closestPointTriangle(p, t) {
  ...
  if (va <= 0 && d4 - d3 >= 0 && d5 - d6 >= 0)
    return add(t.b, mul(sub(t.c, t.b), (d4 - d3) / (d4 - d3 + (d5 - d6))));
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (isShell) {
      ...
      if (distToOrigin > 1e-6) {
        ...
        finalP = [
          closest[0] * (targetDist / distToOrigin),
          closest[1] * (targetDist / distToOrigin),
          closest[2] * (targetDist / distToOrigin),
        ];
      }
    }
    res[i * 3] = finalP[0];
...
function projectToGeometry(points, type, params, applyStlConstraint = true) {
  ...
  if (type === 'sphere') {
    ...
    return tf.concat([xzProj.slice([0, 0], [-1, 1]), yClamped, xzProj.slice([0, 1], [-1, 1])], 1);
...
function draw() {
  ...
  const projected = [];
  const densities =
    state.metrics.densities.length > 0 ? state.metrics.densities : new Array(numPoints).fill(1);
...
function drawDistanceMatrix(pointsArr, numPoints, chartX, chartY, chartW, chartH) {
  ...
  if (numPoints > maxDisplay) {
    ...
    ctx.fillText(`showing ${maxDisplay} of ${numPoints} points`, originX, originY + N * cell + 14);
  }
...
async function setupWebXR() {
  if (!WebXRViewer.isSupported()) {
    els.xrStatus.textContent = 'WebXR not available in this browser.';
    return;
  }
  // Provide the viewer with a live snapshot of the current scene.
  xrViewer = new WebXRViewer(() => {
    let triangles = null;
    if (
      (state.showTriangulation || state.showSolidFill) &&
      state.points &&
      state.params.geometry === 'sphere'
    ) {
      try {
        triangles = getTriangles(state.points.dataSync());
      } catch (e) {
        triangles = null;
      }
    }
    return {
      points: state.points,
      densities: state.metrics.densities,
      geometry: state.params.geometry,
      triangles,
    };
  });
  xrViewer.onEnd(() => {
    els.btnVr.textContent = 'Enter VR';
    els.btnAr.textContent = 'Enter AR';
  });
  const { vr, ar } = await xrViewer.checkSessionSupport();
  if (vr) {
    els.btnVr.disabled = false;
  }
  if (ar) {
    els.btnAr.disabled = false;
  }
  if (vr || ar) {
    els.xrStatus.textContent =
      'Ready: ' + [vr ? 'VR' : null, ar ? 'AR' : null].filter(Boolean).join(' & ');
  } else {
    els.xrStatus.textContent = 'No immersive VR/AR devices detected.';
  }
  const enter = async (mode, btn) => {
    if (xrViewer.session) {
      await xrViewer.end();
      return;
    }
    try {
      await xrViewer.start(mode);
      btn.textContent = mode === 'immersive-ar' ? 'Exit AR' : 'Exit VR';
    } catch (err) {
      console.error(err);
      els.xrStatus.textContent = 'Failed to start session: ' + err.message;
    }
  };
  els.btnVr.addEventListener('click', () => enter('immersive-vr', els.btnVr));
  els.btnAr.addEventListener('click', () => enter('immersive-ar', els.btnAr));
}
